...
export function createCartItem({id, title, description, price, stock, thumbnail, amount}: DataType) {
    return `<li class="cart-products-list__item cart-item">
              ...
              <div class="cart-item__btns">
                <button class="cart-item__decrement" aria-label="decrement-amount" name="decrement" data-stock="${stock}" value="${id}">-</button>
                  <span class="cart-item__amount">${amount}</span>
                <button class="cart-item__increment" value="${id}" data-stock="${stock}" name="increment" aria-label="increment-amount">+</button>
              </div>
            </li>`
}
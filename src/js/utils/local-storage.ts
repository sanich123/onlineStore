...
export function localStorageCouponHelper(coupon: string) {
  const couponsInCart = JSON.parse(
    localStorage.getItem(LS_KEYS.promocode) || "[]"
  );
  if (coupon === PROMOCODES.newYear) {
    if (!couponsInCart) {
      localStorage.setItem(
        LS_KEYS.promocode,
        JSON.stringify([NEW_YEAR_COUPON])
      );
    } else {
      if (
        !JSON.stringify(couponsInCart).includes(JSON.stringify(NEW_YEAR_COUPON))
      ) {
  ...
  if (coupon === PROMOCODES.stupidSanta) {
    if (!couponsInCart) {
      ...
      if (
        !JSON.stringify(couponsInCart).includes(JSON.stringify(SANTA_COUPON))
      ) {
...
export function setAppliedToCoupons(
  coupons: CouponsType[],
  couponName: string
) {
